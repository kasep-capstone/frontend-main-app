import React from 'react';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Shield } from 'lucide-react';
import { UserProfile } from '@/types/profile';

interface AccountSectionProps {
  formData: UserProfile;
}

export const AccountSection: React.FC<AccountSectionProps> = ({ formData }) => {
  const isGoogleAccount = formData.authProvider === 'google';
  
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle className="text-lg flex items-center gap-2">
          <Shield className="w-5 h-5 text-amber-600" />
          Akun Google
        </CardTitle>
        <CardDescription>
          Kelola akun Google Anda yang terhubung
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-3">
          {isGoogleAccount ? (
            /* Connected Google Account */
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center gap-3">
                <svg className="w-5 h-5 text-green-600" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                <div className="flex-1">
                  <div className="font-medium text-green-800">Google Account Terhubung</div>
                  <div className="text-sm text-green-600">{formData.email}</div>
                </div>
                <div className="bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full font-medium">
                  Aktif
                </div>
              </div>
            </div>
          ) : (
            /* Local Account - Google Not Connected */
            <div className="p-3 bg-gray-50 border border-gray-200 rounded-lg opacity-60">
              <div className="flex items-center gap-3">
                <svg className="w-5 h-5 text-gray-400" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                  <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                  <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                  <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
                </svg>
                <div className="flex-1">
                  <div className="font-medium text-gray-600">Akun Lokal</div>
                  <div className="text-sm text-gray-500">{formData.email}</div>
                </div>
                <div className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full font-medium">
                  Local
                </div>
              </div>
              <div className="mt-3 text-sm text-gray-500">
                Akun ini menggunakan sistem autentikasi lokal, bukan Google.
              </div>
            </div>
          )}
        </div>
      </CardContent>
    </Card>
  );
}; 